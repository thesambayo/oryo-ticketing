...
export class StaffService {
	...
	bdmStaff = computed(() =>
		this.staffList().filter((staff) => staff.department.name === Department.BUSINESS_DEVELOPMENT)
	);
	...
	getStaff(department?: Department) {
		const params = department ? new HttpParams().append("department", department ?? "") : new HttpParams();
		return this._http
