...
export class NocMainDashboardComponent {
  ...
  createCompantForm = this._fb.nonNullable.group({
    ...
    endDate: this._fb.nonNullable.control({ value: '', disabled: true }, [
      Validators.required,
    ]),
  });
  ...
  onSubmit() {
    ...
    this._vehiclesService.getVehiclesByParams(payload).subscribe({
      next: (res) => {
        this.generatedReportsResponse = res.data.map<GeneratedClientReport>(
          (resData) => ({
            id: resData.id,
            company_name: resData.company_name,
            total_vehicles:
              this.companies().find((company) => company.id === resData.id)
                ?.total_vehicles ?? 0,
