...
import { ClientVehicle, VehicleReport, VehiclesGlobalReport } from "../features/noc/noc.model";
...
export class VehiclesService {
	...
	getVehiclesByStatus(status = "total") {
		return this._http.get<ApiResponse<ClientVehicle[]>>(`${this.apiURL}/vehicles?sensorStatus=` + status);
	}
	...
	getVehiclesByParams(paramsArgs: GetVehiclesParams) {
		...
		return this._http.get<ApiResponse<ClientVehicle[]>>(`${this.apiURL}/vehicles`, { params });
	}
	...
	convertToCSV(objArray: Record<string, string>[], allowedHeaders: Record<string, string>[]): string {
		let str = "";
